...
Class Oferta {
	...
	private function prepareEmbed($v) {
		...
		$m->tytul = $v->title;
		$m->opis_short = Core::getTitle($v->tresc, 100);
		$m->opis = $v->tresc;
		...
		/* links */
		$m->link =  get_lang_url('oferty/wyswietl/' . $v->id . '/' . create_url($m->tytul, 50));
				...
				<a href="{$m->link}" title="{$this->texts['wiecej_info']}">
					<div class="row">
						<div class="col-xs-5 col-sm-5 col-md-4">
							<img class="img-responsive lazy" data-src="{$m->image}" alt="zdjecie glowne oferty" />
						</div>
		...
		$html = '<div class="gallery" data-url="'.$url.'">
			<div class="preview row">
				...
				<div class="col-xs-12">
					...
					<img class="img-responsive" src="'.$first_url.'" alt="galeria" />
				</div>
			</div>
		...
		if (count($images) > 1)
			foreach ($images as $k => $v) {
				...
				$image = '<div class="img col-xs-4 '.$klasa.'">
					<a href="'.$full_img.'" data-url="'.$folder.'/'.$v->url.'"><img class="img-responsive lazy" data-src="'.$img.'" /></a>
				</div>';
				$html .= $image;
			}
