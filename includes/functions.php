<?php
	...
	function lang_url($url='') {
		echo __SITE_PATH . ( (LANG == 'pl') ? '' : LANG . '/' ) . $url;
	}

	function get_lang_url($url='') {
		return __SITE_PATH . ( (LANG == 'pl') ? '' : LANG . '/' ) . $url;
	}
	...
	class Core {
		...
		public static function print_menu($menu, $klasa, $uppercase = false) {
			$html = '<ul class="menu '.$klasa.'">';
			foreach ($menu as $key => $value) {
				$temp = $value->strona_id == 1 ? '' : 'strony/' . $value->short;
				if (strpos($value->short,'/') !== false) $temp = $value->short;
				$active = strpos($_SERVER['REQUEST_URI'], $value->short) !== false ? 'active' : '';
				$tyt = $uppercase == true ? mb_strtoupper($value->title) : $value->title;
				$html .= '<li class="'.$active.'"><a href="'.get_lang_url($temp).'">'.$tyt.'</a></li>';
			}
			$html .='</ul>'; 
			echo $html;
			return $html;
		...
		public static function isHome() {
			if (LANG == 'pl')
				...
				return $_SERVER['REQUEST_URI'] == '/' . LANG . '/';
		}
		...
		public static function shortDescription($fullDescription, $initialCount=180) {
			$shortDescription = '';
		...
		public static function loginRequired($break = false) {
			if (!logged):
				if ($break) exit;
				header ('location: /autoryzacja/logowanie');
			endif;
		}
		...
		public function sendMail($to, $temat, $tresc, $zalacznik=null, $zal_name=null) {
			...
			$mail->Host = 'tnjestate.pl';  							// Specify main and backup SMTP servers
			$mail->SMTPAuth = true;                               // Enable SMTP authentication
			$mail->Username = 'automat@tnjestate.pl';                 // SMTP username
			$mail->Password = '7Sm7J#0FLe77';                           // SMTP password
			$mail->SMTPSecure = 'ssl';                            // Enable encryption, 'ssl' also accepted
			$mail->Port = "465";

			$mail->From = 'automat@tnjestate.pl';
			$mail->FromName = 'TNJ Estate - biuro nieruchomośći';
		...
		public static function checkCaptcha() {
			return $_SESSION['captcha']['code']  == strtolower($_POST['captcha']); 
		}
